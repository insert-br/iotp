...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, **opts):
        ...
        link_sort_key = lambda x: int(re.sub(r'[^\d]+', '', x['node2'] + x['node1']))
        ...
        self.printPortMapping(log_file="%s/%s.json" %(log_dir, 'port_mapping'))           
    ...
    def printPortMapping(self, log_file=None):        
        ...
        if log_file:
            with open(log_file, 'w') as fout:
                json.dump(self.sw_port_mapping, fout, sort_keys=True)
            print "  Port mapping saved in %s" %(log_file)     
...
class ExerciseRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
                       switch_json, bmv2_exe='simple_switch', quiet=False, script=None):
        """ Initializes some attributes and reads the topology json. Does not
            ...
            Arguments:
                ...
                script : string       // script to run inside mininet
        """
        self.script = script
        self.quiet = quiet
    ...
    def run_exercise(self):
        ...
        self.program_hosts()
        self.program_switches(log_file='%s/%s.json' %(self.log_dir, 'sw_control_ports'))
    ...
    def program_switch_p4runtime(self, sw_name, sw_dict):
        ...
        self.sw_control_ports[sw_name]['device_id'] = device_id       
        self.sw_control_ports[sw_name]['grpc'] = grpc_port        
    ...
    def program_switch_cli(self, sw_name, sw_dict):
        ...
        self.sw_control_ports[sw_name]['thrift'] = thrift_port

        cli_input_commands = sw_dict['cli_input']
        self.logger('Configuring switch %s using CLI with file %s' % (sw_name, cli_input_commands))
        self.logger('%s -> Thrift port: %s' % (sw_name, str(thrift_port)) )
    ...
    def program_switches(self, log_file=None):
        ...
        if log_file:
            with open(log_file, 'w') as fout:
                json.dump(self.sw_control_ports, fout, sort_keys=True)
            print "  Switch control ports saved in %s" %(log_file)     
    ...
    def do_net_cli(self):
        ...
        print('')
        if self.switch_json or self.cli:
        ...
        CLI(self.net, script=self.script)
...
def get_args():
    ...
    parser.add_argument('-s', '--script', help='Script to run inside Mininet',
...
if __name__ == '__main__':
    ...
    exercise = ExerciseRunner(args.topo, args.log_dir, args.pcap_dir,
                              args.switch_json, args.behavioral_exe, args.quiet, args.script)
